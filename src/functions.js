...
export const getFloatVal = ( string ) => {
	...
	let floatValue = string.match( /[+-]?\d+(\.\d+)?/g ).toString().replace(/,/g,'');
...
export const getFormattedCart = ( data ) => {
	...
	//calculate one product at a time, pushing to formattedCart as the whole set of data
	for( let i = 0; i < givenProducts.length; i++  ) {
		const product = {};
		if ( null === givenProducts?.[ i ]?.variation ) {
			var givenProduct = givenProducts?.[ i ]?.product?.node;

		} else {
			var givenProduct = givenProducts?.[ i ]?.variation?.node;
